...
만약 4방향으로 모두 갈 수 있었다면 사이클이 생기기 때문에 다익스트라 알고리즘을 사용해서 해결해야 한다. 하지만 음의 간선이 있는경우에는 다익스트라를 적용하지 못한다.
비슷한 문제로 1937-욕심쟁이판다 문제에서는 value가 더 높은 곳으로만 이동해야 하기 때문에 사이클이 생기지 않는 조건이 생긴다.
